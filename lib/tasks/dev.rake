...
namespace :dev do
  ...
  task :fake_event_and_registrations => :environment do
    event = Event.create!(:status => "public", :name => "Rails Meetup", :friendly_id => "rails-meetup")
    t1 = event.tickets.create!(:name => "Guest", :price => 0)
    t2 = event.tickets.create!(:name => "VIP One", :price => 199)
    t3 = event.tickets.create!(:name => "VIP Two", :price => 199)
    ...
    puts "Let's visit http://localhost:3000/admin/events/rails-meetup/registrations"
  end

  task :import_registration_csv_file => :environment do
    event = Event.find_by_friendly_id("rails-meetup")
